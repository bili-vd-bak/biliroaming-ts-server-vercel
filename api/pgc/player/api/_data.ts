...
const checkRegion = (region: env._block_region) => {
  for (let i of env.block_region) {
...
export const middleware = async (
  ...
  //参数验证
  if (!data.area) return [false, 7]; //缺少地区参数
  if (!checkRegion(data.area as env._block_region)) return [false, 8]; //检测区域
...
export const main = async (url_data: string) => {
  ...
  const url = new URL(url_data, env.api_playurl);
  const data = qs.parse(url.search.slice(1));
  const api_playurl = env.plugin_conf[data.area as string]
    ? env.plugin_conf[data.area as string]
    : env.api_playurl; //TODO 此时应按顺序向所有配置的API请求
  ...
  if (env.NOTION_db_cache) {
    ...
    if (rCache.code == 0) {
      if (rCache.visit_time + env.cache_time < bili.UTC()) {
        ...
        const res = await fetch(api_playurl + url_data).then((res) =>
          res.json()
        );
      ...
      //无缓存
      const res = await fetch(api_playurl + url_data).then((res) => res.json());
      if (res.code == 0) await addNewCache(url_data, res);
      return res;
    }
    //从以下开始执行
  } else return await fetch(api_playurl + url_data).then((res) => res.json());
};
