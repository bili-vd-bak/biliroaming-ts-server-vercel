...
export const query = async (UID: number) => {
  const res = await db.query(env.NOTION_db_blacklist, JSON.stringify({
    filter: {
      and: [
        {
          property: 'UID',
...
export const addNewUser = async (UID: number, type: 0 | 1 | 2 = 0, reason = '', ban_until = 0) => {
  const res = await db.update(env.NOTION_db_blacklist, JSON.stringify({
    parent: { database_id: env.NOTION_db_blacklist },
    properties: {
      UID: {
        id: 'title', type: 'title',
        title: [{ type: 'text', text: { content: UID.toString() }, plain_text: UID.toString() }]
      },
      type: {
        number: type
      },
      reason: {
        rich_text: [{ type: 'text', text: { content: reason }, plain_text: reason }]
      },
      ban_until: {
        number: ban_until
      }
...
export const checkIfFinishBan = async (data): Promise<boolean> => {
  if (data.results[0]) {
    ...
    if (bili.UTC() >= ban_until) {
      await db.edit(id, JSON.stringify({
        properties: {
          type: {
            number: 0
          },
  ...
  if (res.code == 0) {
    ...
    if (env.public_blacklist_enabled) {
      const data = await fetch(env.public_blacklist + UID)
        .then(res => res.json())
        .catch(err => console.error(err))
      await addNewUser(UID, data.data.status, data.data.is_blacklist ? '本用户已进入公共黑名单' : '', data.data.ban_until)
      return data
    } else {
